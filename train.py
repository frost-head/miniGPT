...
# history_weights = './checkpoints/historysp.pth' # history of training. 
tokenizer_path = './large_tokenizer.json'

model_weights = './checkpoints/modelsp.pth' # weights for model
optim_weights = './checkpoints/optimsp.pth' # optimizer Weights
history_weights = './checkpoints/historysp.pth' # history of training. 

model_save = "./checkpoints/modelsp.pth"
optim_save = "./checkpoints/optimsp.pth"
history_save = "./checkpoints/historysp.pth"
...
for epoch in range(4000):
    ...
    for batch_idx, a in enumerate(train_dataloader):
        ...
        if (batch_idx+1) % ((max_no_tokens / modelArguments.max_batch_size)*32) == 0 or math.isnan(loss):
            ...
            torch.save(fmodel.state_dict(), model_save)
            torch.save(optimizer.state_dict(), optim_save)
            torch.save(history, history_save)
